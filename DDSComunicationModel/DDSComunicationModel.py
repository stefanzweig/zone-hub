...
class ReaderListener(fastdds.DataReaderListener):
    ...
    def on_subscription_matched(self, datareader, info):
        if 0 < info.current_count_change:
            print(
                "log replay Subscriber matched publisher {}".format(
                    info.last_publication_handle
                )
            )
        else:
            print(
                "log replay unmatched publisher {}".format(
                    info.last_publication_handle
                )
            )
...
class DDSComunication(BaseNode):
    ...
    def _fastdds_reader_init(self):
        ...
        self.linParserData_topic_data_type.setName(
            BaseNodeData.linParserTopic.IDL
        )
        self.linParserData_type_support = fastdds.TypeSupport(
            self.linParserData_topic_data_type
        )
        self.participant.register_type(self.linParserData_type_support)
        self.topic_lin_parser = self.participant.create_topic(
            BaseNodeData.linParserTopic.NAME,
            self.linParserData_topic_data_type.getName(),
            self.topic_qos,
        )
        self.listener_lin_parser = ReaderListener(
            self, linParserData.linFrames, self._lin_frame_callback
        )
        self.reader_lin = self.subscriber.create_datareader(
            self.topic_lin_parser, self.reader_qos, self.listener_lin_parser
        )

        ###someip package calling topic
        self.someIpParserData_topic_data_type = (
            SomeIpParserData.framePubSubType()
        )
        self.someIpParserData_topic_data_type.setName(
            BaseNodeData.someipPackage.IDL
        )
        self.someIpParserData_type_support = fastdds.TypeSupport(
            self.someIpParserData_topic_data_type
        )
        ...
        self.topic_someIp_package_parser = self.participant.create_topic(
            BaseNodeData.someipPackage.NAME,
            self.someIpParserData_topic_data_type.getName(),
            self.topic_qos,
        )
        self.listener_someIp_package_parser = ReaderListener(
            self, SomeIpParserData.frame, self._someip_package_callback
        )
        self.reader_someip_package = self.subscriber.create_datareader(
            self.topic_someIp_package_parser,
            self.reader_qos,
            self.listener_someIp_package_parser,
        )

        self.topic_someIp_calling_parser = self.participant.create_topic(
            BaseNodeData.someIpCalling.NAME,
            self.someIpParserData_topic_data_type.getName(),
            self.topic_qos,
        )
        self.listener_someIp_calling_parser = ReaderListener(
            self, SomeIpParserData.frame, self._someip_calling_callback
        )
        self.reader_someip_calling = self.subscriber.create_datareader(
            self.topic_someIp_calling_parser,
            self.reader_qos,
            self.listener_someIp_calling_parser,
        )

        # can uds topic
        self.canUdsMessageData_topic_data_type = (
            canMessageData.canMessagePubSubType()
        )
        self.canUdsMessageData_topic_data_type.setName(
            BaseNodeData.canUdsTopic.IDL
        )
        self.canUdsMessageData_type_support = fastdds.TypeSupport(
            self.canUdsMessageData_topic_data_type
        )
        self.participant.register_type(self.canUdsMessageData_type_support)
        self.topic_can_uds = self.participant.create_topic(
            BaseNodeData.canUdsTopic.NAME,
            self.canUdsMessageData_topic_data_type.getName(),
            self.topic_qos,
        )
        self.listener_can_uds = ReaderListener(
            self, canMessageData.canMessage, self._can_uds_callback
        )
        self.reader_can_uds = self.subscriber.create_datareader(
            self.topic_can_uds, self.reader_qos, self.listener_can_uds
        )
